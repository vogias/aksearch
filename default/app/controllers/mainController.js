...
listing
		.controller(
				...
				function($rootScope, $scope, $http, $routeParams, $location,
						$modal, $log, sharedProperties) {

					// $scope.conf_file = drupalVariables.root +
					// '/config/conf.json';
					// $scope.conf_file = drupalVariables.root + '/config?n=' +
					// drupalVariables.nodeid;
					$scope.conf_file = drupalVariables.base + '/akconf/'
							+ drupalVariables.nodeid;
					$scope.lang_path = drupalVariables.root + '/config/lang/';

					var mappings_file = drupalVariables.root
							+ '/config/facets_mappings.json';
					...
					$http
							...
							.error(
									function(err) {
										...
										// Page Size defines the number of
										// results per page
										$scope.pageSize = 15;
										// Selected Language
										...
										$scope.facets = [ 'set' ];
										$scope.limit_facets = {}; // {"set":["oeintute","prodinraagro"],
																	// "language":["en","fr"]};
																	// // limit
																	// facets
										$scope.limit_facets_number = 10; // limits
																			// the
																			// number
																			// of
																			// the
					...
					// at Initialization searches '*' see:listingController >
					// if(init)
					$rootScope.query = "";
					...
					/*
					 * @function app_init(language) : Initialize app @param
					 * language {string} : defines the language of the app
					 */
					...
					$scope.app_init = function(language) {
						...
						$http.get($scope.lang_path + $scope.lang_file).success(
								function(data) {
					...
					/*
					 * @function init_finder(schema, facets_type) : Initialize
					 * Finder @param schema {string} : defines the schema of the
					 * finder - options: 'akif', 'agrif' @param facets_type
					 * {string} : defines what facets we want in every case
					 */
					$scope.init_finder = function(schema, facets_type) {

						// When defining the facet_type we want to use, we also
						// can limit everyone of these facets and define a
						// specific mapping file for facets.
						// For now we have 'training', 'educational',
						// 'publications'.
						// We can easily add more just by adding a new case in
						// the following switch, and also add other options
						// related to facets_type
						switch (facets_type) {
						case 'educational':
							// $scope.facets =
							// ['language','learningResourceTypes','contexts','endUserRoles'];
							// /* 'set' */
							// $scope.limit_facets =
							// {"set":["oeellinogermaniki", "aglrmiksike",
							// "edumnhn"]}; //"set":["aglrnedunhmc"]
							mappings_file = drupalVariables.root
									+ '/config/educational_facets_mappings.json';
							break;
						default:
						...
						// want to have in the snippets.
						// Here can be added more schemas and also other options
						// related to it.
						switch (schema) {
						case 'akif':
					...
					/*
					 * @function submit(type): query submission @param type :
					 * 'educational', 'publications', 'training' different types
					 * leads to different paths in Routing
					 */
					...
					/*
					 * @function resetLocation() : function for cleaning up the
					 * location
					 */
					 ...
					 * @param: lang code xx (must be the same with the language
					 * file in config/lang/xx.json)
					 */
					$scope.change_language = function(lang) {
						$scope.selectedLanguage = lang;

						$http
								.get($scope.lang_path + lang + '.json')
								.success(
										function(data) {
											$rootScope.languages = data;
					...
					/*
					 * @function truncate() : function for truncate long texts
					 * (i.e. description in listing) @param text {string}: text
					 * to sanitize
					 */
					$scope.truncate = function(str, maxLength, suffix) {
						if (str.length > maxLength) {
							str = str.substring(0, maxLength + 1);
							str = str.substring(0, Math.min(str.length, str
									.lastIndexOf(" ")));
							str = str + suffix;
						}
						return str;
					...
					/*
					 * @function scrollToTop() : function for scrolling to top
					 * of the page helpful when we have infinite vertical scroll
					 */
					$scope.scrollToTop = function() {
						...
						var animateScroll = function() {
							...
							var val = Math.easeInOutQuad(currentTime, start,
									change, duration);
					 ...
					 * @function easeInOutQuad(t, b, c, d) @param t = current
					 * time @param b = start value @param c = change in value
					 * @param d = duration
					 */
